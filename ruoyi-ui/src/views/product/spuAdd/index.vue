        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="商品介绍" prop="decript">
              <image-upload :limit="5" v-model="spu.decript" :value-type="'Array'"></image-upload>
            </el-form-item>

            <el-form-item label="商品图集" prop="images">
              <image-upload :limit="10" v-model="spu.images" :value-type="'Array'"></image-upload>
            </el-form-item>
            <el-form-item>
              ...
              <template v-slot="scope">
                <el-row>
                  <el-col :span="24">
                    ...
                    <image-upload :limit="5" v-model="uploadImages" :value-type="'Array'"/>
                  </el-col>
                  <el-col :span="24">
...
import { listMemberLevel } from "@/api/member/memberLevel";
import { listAttrGroupAttrs } from "@/api/product/attrGroup";
import { addSpu } from "@/api/product/spu";
  ...
  methods: {
    ...
    getMemberLevels() {
      listMemberLevel().then(response => {
        this.dataResp.memberLevels = response.rows;
      });
    },
    showInput(idx) {
    ...
    generateSkus() {
      ...
      this.spu.skus = skus;
      console.log("结果!!!", this.spu, this.spu.skus, this.dataResp.tableAttrColumn);
    },
    ...
    getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        listAttr({
          catalogId: this.spu.catalogId,
          attrType: 0
        }).then(response => {
          this.dataResp.saleAttrs = response.rows;
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        listAttrGroupAttrs(this.spu.catalogId).then(response => {
          let data = response.data;
          //先对表单的baseAttrs进行初始化
          data.forEach(item => {
            let attrArray = [];
            if(!item.attrs){
              console.log('no attrs:',item);
              return;
            }
